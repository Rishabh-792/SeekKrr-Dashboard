...
class TravelDashboard {
    constructor() {
        ...
        this.chartColors = [this.colors.cyan, this.colors.yellow, this.colors.magenta, this.colors.green, this.colors.orange, this.colors.purple];
        this.init();
    }
    ...
    async init() {
        ...
        try {
            ...
            document.getElementById('loadingScreen').innerHTML = '<h2>Failed to load data. Please try again later.</h2>';
        }
    }
    ...
    setupEventListeners() {
        ...
        document.querySelector('.modal-overlay')?.addEventListener('click', () => this.closeFilterModal());
    ...
    showDashboard() {
        setTimeout(() => { document.getElementById('dashboard').classList.remove('hidden'); }, 500);
    }
    ...
    animatePanels() {
        setTimeout(() => {
            ...
            panels.forEach((panel, index) => {
                setTimeout(() => { panel.classList.add('animate'); }, index * 150);
            });
        }, 800);
    }
     ...
     */
    updateKPIs() {
        const totalResponses = Object.values(this.filteredData.demographics.age_groups).reduce((a, b) => a + b, 0);
        const primaryAge = this.getTopKey(this.filteredData.demographics.age_groups);
        const topProblem = this.getTopKey(this.filteredData.insights.problems_faced);

        document.getElementById('kpi-total-responses').dataset.target = totalResponses;
        document.getElementById('kpi-primary-age').textContent = primaryAge.replace(' years', '');
        document.getElementById('kpi-top-problem').textContent = topProblem;
        
        document.querySelectorAll('.kpi-value[data-target]').forEach(el => el.textContent = '0');
        this.animateKPIs();
    }
    ...
    initializeCharts() {
        ...
        setTimeout(() => {
            // New Charts
            this.createIndependenceChart();
            this.createDesireVsFearChart();
        ...
        this.charts.independence = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{ 
                    ...
                    backgroundColor: [this.colors.cyan, this.colors.magenta],
                    hoverBackgroundColor: [this.colors.cyan + 'CC', this.colors.magenta + 'CC'],
                    barThickness: 50
                }]
            },
    ...
    createDesireVsFearChart() {
        ...
        if (!ctx) return;
        const experiences = Object.entries(this.filteredData.insights.experiences_sought).sort((a,b) => b[1]-a[1]).slice(0,3);
        const problems = Object.entries(this.filteredData.insights.problems_faced).sort((a,b) => b[1]-a[1]).slice(0,3);
            ...
            options: this.getChartOptions('bar', true, {
                label: (context) => {
                    ...
                    if (datasetLabel.includes('Desires')) {
                        return `${datasetLabel}: ${experiences[context.dataIndex][0]}`;
                    }
                    if (datasetLabel.includes('Fears')) {
                        return `${datasetLabel}: ${problems[context.dataIndex][0]}`;
                    }
                    return '';
                }
            })
    ...
    createAgeChart() {
        ...
        this.charts.age = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{ data: Object.values(data), backgroundColor: this.chartColors }]
            },
    ...
    createProfessionChart() {
        ...
        this.charts.profession = new Chart(ctx, {
            ...
            data: {
                labels: Object.keys(data),
                datasets: [{ data: Object.values(data), backgroundColor: this.chartColors.concat(['#b1b1b1', '#d1d1d1']) }]
            },
            options: this.getChartOptions('doughnut')
        });
    }
    ...
    createFrequencyChart() {
        ...
        this.charts.frequency = new Chart(ctx, {
            ...
            data: {
                labels: Object.keys(data),
                datasets: [{ label: 'Travelers', data: Object.values(data), backgroundColor: this.colors.cyan }]
            },
    ...
    createBudgetChart() {
        ...
        this.charts.budget = new Chart(ctx, {
            ...
            data: {
                labels: Object.keys(data),
                datasets: [{ data: Object.values(data), backgroundColor: [this.colors.green, this.colors.yellow, this.colors.orange, this.colors.red] }]
            },
            options: this.getChartOptions('doughnut')
        });
    }
    ...
    createCompanionsChart() {
        ...
        this.charts.companions = new Chart(ctx, {
            ...
            data: {
                labels: Object.keys(data),
                datasets: [{ label: 'Preferences', data: Object.values(data), backgroundColor: this.colors.yellow }]
            },
            options: this.getChartOptions('bar')
        });
    }
    ...
    createExplorationChart() {
        ...
        this.charts.exploration = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: Object.keys(data).map(label => this.wrapText(label, 20)),
                datasets: [{ label: 'Methods', data: Object.values(data), backgroundColor: this.colors.purple }]
            },
    ...
    createExperiencesChart() {
        ...
        this.charts.experiences = new Chart(ctx, {
            ...
            data: {
                labels: Object.keys(data),
                datasets: [{ label: 'Interest Level', data: Object.values(data), backgroundColor: this.colors.magenta }]
            },
            options: this.getChartOptions('bar')
        });
    }
    ...
    createProblemsChart() {
        ...
        this.charts.problems = new Chart(ctx, {
            ...
            data: {
                ...
                datasets: [{ label: 'Frequency', data: sortedData.map(([, value]) => value), backgroundColor: this.colors.red }]
            },
            options: this.getChartOptions('horizontalBar')
        });
    }
    ...
    createSatisfactionCharts() {
        const satisfactionColors = ['#ff7b72', '#f08a5d', '#e3b341', '#88d8b0', '#56d364'];
        const frustrationColors = ['#56d364', '#88d8b0', '#e3b341', '#f08a5d', '#ff7b72'];
        const overspendingColors = ['#56d364', '#88d8b0', '#e3b341', '#f08a5d', '#ff7b72'];
        const metrics = this.filteredData.satisfaction_metrics;
        
        const createDoughnut = (id, labels, data, colors) => {
            const ctx = document.getElementById(id);
            if(ctx) this.charts[id] = new Chart(ctx, { type: 'doughnut', data: { labels: labels, datasets: [{ data: data, backgroundColor: colors, borderWidth: 2, borderColor: '#161b22' }] }, options: this.getChartOptions('doughnut') });
        };

        createDoughnut('satisfactionChart', ['Very Poor', 'Poor', 'Average', 'Good', 'Excellent'], Object.values(metrics.satisfaction_scores), satisfactionColors);
        createDoughnut('frustrationChart', ['Very Low', 'Low', 'Moderate', 'High', 'Very High'], Object.values(metrics.frustration_levels), frustrationColors);
        createDoughnut('missingChart', ['None', 'Few', 'Some', 'Many', 'Most'], Object.values(metrics.missing_experiences), this.chartColors);
        createDoughnut('overspendingChart', ['Never', 'Rarely', 'Sometimes', 'Often', 'Always'], Object.values(metrics.overspending), overspendingColors);
    }
    ...
    getChartOptions(type, showLegend = true, tooltipCallbacks = {}) {
        const options = {
            ...
            plugins: {
                legend: { 
                    ...
                    position: 'bottom', 
                    labels: { color: this.colors.text, padding: 15, usePointStyle: true, font: { size: 12 } } 
                },
                tooltip: {
        ...
        if (type.includes('bar')) {
            ...
            options.scales = {
                x: { ticks: { color: this.colors.text, font: {size: 11 } }, grid: { color: this.colors.grid } },
                y: { ticks: { color: this.colors.text, font: {size: isHorizontal ? 10 : 11 } }, grid: { color: this.colors.grid } }
            };
    ...
    openFilterModal() { document.getElementById('filterModal').classList.remove('hidden'); }
    ...
    exportData() {
        const dataStr = JSON.stringify({ timestamp: new Date().toISOString(), data: this.filteredData }, null, 2);
